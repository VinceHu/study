...
> 强缓存（Cache-Control/Expires）不发请求直接用缓存，协商缓存（ETag/Last-Modified）发请求验证，未修改返回 304。流程是先强缓存，过期了再协商缓存。
...
>
> **强缓存**是指浏览器直接使用本地缓存，不发请求到服务器。通过 `Cache-Control: max-age=3600` 或 `Expires` 控制。在缓存有效期内，浏览器直接从缓存读取，状态码是 200（from cache）。
>
> **协商缓存**是指缓存过期后，浏览器发请求到服务器验证资源是否修改。通过 `ETag/If-None-Match` 或 `Last-Modified/If-Modified-Since` 实现。如果资源没变，服务器返回 **304 Not Modified**，浏览器继续用缓存；如果变了，返回 200 和新资源。
>
> 流程是：先检查强缓存，有效就直接用；过期了再走协商缓存。
>
> 实际应用中，**静态资源**（JS、CSS、图片）用强缓存 + 文件指纹（hash），设置很长的 max-age，文件内容变了 hash 就变，URL 变了就重新请求。**HTML** 用协商缓存或 no-cache，保证能拿到最新的入口文件。
>
> ETag 比 Last-Modified 更精确，Last-Modified 只能精确到秒，而且有些情况文件内容没变但修改时间变了。"
